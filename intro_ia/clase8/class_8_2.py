...
#%%
    # n = 2
    X2 = np.hstack((np.power(X_train, 2),X_train))
    mse_list, w_array = k_folds(X2, y_train, k=5)
    index_w = selector(mse_list, w_array)
    ...
    # n = 3
    X3 = np.hstack((np.power(X_train, 3), np.power(X_train, 2),X_train))
    ...
    # n = 4
    X4 = np.stack((np.power(X_train, 4), np.power(X_train, 3), np.power(X_train, 2),X_train))
